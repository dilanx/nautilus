...
static int flush(void *state)
{
    ...
    //xfer_req.offset = (uint64_t) d->frame_buffer;
...
static inline int graphics_draw_pixel(void *state, nk_gpu_dev_coordinate_t *location, nk_gpu_dev_pixel_t *pixel)
{
    ...
    uint32_t x_min = d->frame_box.x;
    uint32_t y_min = d->frame_box.y;
    uint32_t x_max = x_min + d->frame_box.width - 1;
    uint32_t y_max = y_min + d->frame_box.height - 1;

    if (x < x_min || x > x_max || y < y_min || y > y_max) {
        DEBUG("outside of frame box :(\n");
        return 0;
    }

    x_min = d->clipping_box.x;
    y_min = d->clipping_box.y;
    x_max = x_min + d->clipping_box.width - 1;
    y_max = y_min + d->clipping_box.height - 1;

    if (x < x_min || x > x_max || y < y_min || y > y_max) {
        DEBUG("outside of clipping box :(\n");
        return 0;
    }
...
static inline int graphics_draw_line(void *state, nk_gpu_dev_coordinate_t *start, nk_gpu_dev_coordinate_t *end, nk_gpu_dev_pixel_t *pixel)
{
    ...
    int x = x0;
    int y = y0;
    graphics_draw_pixel(state, start, pixel);
    ...
    if (abs_dx > abs_dy) {
        ...
        for (int i = 0; i < abs_dx; i++) {
            ...
            graphics_draw_pixel(state, &(nk_gpu_dev_coordinate_t){x,y}, pixel);
        }
    } else {
...
static int graphics_draw_poly(void *state, nk_gpu_dev_coordinate_t *coord_list, uint32_t count, nk_gpu_dev_pixel_t *pixel)
{
    ...
    if (count == 1) {
        graphics_draw_pixel(state, &coord_list[0], pixel);
        return 0;
    }

    for (int i = 1; i < count; i++) {
        graphics_draw_line(state, &coord_list[i - 1], &coord_list[i], pixel);
    }
    graphics_draw_line(state, &coord_list[0], &coord_list[count - 1], pixel);
...
static int graphics_fill_box_with_pixel(void *state, nk_gpu_dev_box_t *box, nk_gpu_dev_pixel_t *pixel, nk_gpu_dev_bit_blit_op_t op)
{
    ...
    for (uint32_t x = box->x; x < box->x + box->width; x++) {
        for (uint32_t y = box->y; y < box->y + box->height; y++) {
            if (x < d->frame_box.x || x > d->frame_box.x + d->frame_box.width - 1 ||
                y < d->frame_box.y || y > d->frame_box.y + d->frame_box.height - 1) {
                continue;
            }
            if (x < d->clipping_box.x || x > d->clipping_box.x + d->clipping_box.width - 1 ||
                y < d->clipping_box.y || y > d->clipping_box.y + d->clipping_box.height - 1) {
                continue;
            }
...
static int graphics_fill_box_with_bitmap(void *state, nk_gpu_dev_box_t *box, nk_gpu_dev_bitmap_t *bitmap, nk_gpu_dev_bit_blit_op_t op)
{
    ...
    for (uint32_t x = box->x; x < box->x + box->width; x++) {
        ...
        for (uint32_t y = box->y; y < box->y + box->height; y++) {
            ...
            uint32_t* buf = (uint32_t*) d->frame_buffer;
            nk_gpu_dev_pixel_t old = {buf[y * d->frame_box.width + x]};
            apply_with_blit(&old, &bitmap->pixels[(by - 1) * bitmap->width + (bx - 1)], op);
            graphics_draw_pixel(state, &(nk_gpu_dev_coordinate_t) {x, y}, &old);
...
static int graphics_copy_box(void *state, nk_gpu_dev_box_t *source_box, nk_gpu_dev_box_t *dest_box, nk_gpu_dev_bit_blit_op_t op)
{
    ...
    int sx = source_box->x;
    int sy = source_box->y;
    int dx = dest_box->x - source_box->x;
    int dy = dest_box->y - source_box->y;
    ...
    for (uint32_t x = dest_box->x; x < dest_box->x + dest_box->width; x++) {
        for (uint32_t y = dest_box->y; y < dest_box->y + dest_box->height; y++) {
            ...
            uint32_t* buf = (uint32_t*) d->frame_buffer;
            nk_gpu_dev_pixel_t old = {buf[y * d->frame_box.width + x]};
            nk_gpu_dev_pixel_t new = {buf[(y - dy) * d->frame_box.width + (x - dx)]};
            apply_with_blit(&old, &new, op);
            graphics_draw_pixel(state, &(nk_gpu_dev_coordinate_t) {x, y}, &old);
        }
    }
